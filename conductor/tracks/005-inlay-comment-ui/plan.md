...
- [ ] Task: CommentInputPanel UI とコールバックの実装
    - [ ] `src/test/kotlin/.../editor/ui/CommentInputPanelTest.kt` を新規作成し、以下を検証するテストを記述する（TDD Red Phase）:
    ...
    - [ ] `src/main/kotlin/.../editor/ui/CommentInputPanel.kt` に `CommentInputPanel`（`JPanel` 継承）を実装し、テストをパスさせる（TDD Green Phase）:
...
- [ ] Task: CommentBlockRenderer (Inlay) の実装
    - [ ] `src/test/kotlin/.../editor/ui/CommentBlockRendererTest.kt` を新規作成し、以下を検証するテストを記述する（TDD Red Phase）:
    ...
    - [ ] `src/main/kotlin/.../editor/ui/CommentBlockRenderer.kt` に `CommentBlockRenderer`（`EditorCustomElementRenderer` 実装）を実装し、テストをパスさせる（TDD Green Phase）:
...
- [ ] Task: 入力パネルと Inlay 表示の連携ロジックの実装
    - [ ] `src/test/kotlin/.../editor/CommentInlayManagerTest.kt` を新規作成し、以下を検証するテストを記述する（TDD Red Phase）:
    ...
    - [ ] `src/main/kotlin/.../editor/CommentInlayManager.kt` に `CommentInlayManager` を実装し、テストをパスさせる（TDD Green Phase）:
        ...
        - Delete 時: すべての Inlay を `Disposer.dispose()`
    - [ ] `src/main/kotlin/.../editor/ReviewCommentGutterIconRenderer.kt` の `getClickAction()` を変更:
...
- [ ] Task: ローカル JSON ストレージ連携
    ...
    - [ ] `CommentInlayManager.kt` の Save/Delete ロジックに `ReviewCommentStorage` 連携を実装し、テストをパスさせる（TDD Green Phase）:
        - `openInputPanel` 呼び出し元（`ReviewCommentGutterIconRenderer`）が `project` を渡し、`ReviewCommentStorage` を初期化
