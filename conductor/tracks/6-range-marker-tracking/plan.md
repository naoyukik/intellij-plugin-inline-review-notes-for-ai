...
- [ ] Task: 保存同期の統合ケースを Red/Green で追加する
  - [ ] `editor/CommentInlayManagerStorageTest.kt` に、単一行と複数行コメントの前後へ行を挿入・削除して保存した行番号を検証する
  - [ ] 同テストに、未解決・解決済みを含む複数コメントを一度の保存で同期するケースを追加する
  - [ ] 同テストに、範囲削除後もコメントが残り、`isOutdated = true` と直近の行番号が保持されるケースを追加する
  - [ ] 作成・編集・削除・再読込とブランチ別保存の既存ケースを実行して回帰がないことを検証する
- [ ] Task: 品質ゲートを実行する
  - [ ] `./gradlew test` を実行する
