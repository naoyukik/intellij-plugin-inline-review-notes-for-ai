...
セキュリティエラーが発生するので `&&` などでコマンドを接続することは禁止する。
    ...
    - PRテンプレート: !{cat "$(git rev-parse --show-toplevel)/.github/pull_request_template.md"}
    - GitHub CLI確認: !{gh version}
    - チケット番号: !{git rev-parse --abbrev-ref HEAD | grep -oE '^[0-9]+'}
    - Actual Code Changes (Diff) ロックファイルなどのノイズを除外し、本質的な変更差分のみを取得する: !{git diff main..HEAD -- . ':(exclude)package-lock.json' ':(exclude)yarn.lock' ':(exclude)*.lock'}
...
5. コマンド生成と実行
   GitHub CLI (`gh`) を使用して Draft PR を作成する。
